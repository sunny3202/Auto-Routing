...
class Importer:
    ...
    def equip_poc_import(path : str) -> list[RoutingEntity]:
        ...
        while True:
            ...
            utility_data = data[1].replace(" ","")

            utility_data = utility_data.upper()
            ...
            # region CS공간 내 설비 POC가 아닌 경우 예외처리
            if pos[0] < 120000:
                continue
            # endregion
            
            #region 상하부 POC에 대하여 시작 방향 지정 (시작방향 제공 시 불필요)
            direction = (0,0,-1)
            if pos[2] > 52000:
                direction = (0,0,1)
            #endregion

            #region FSF구간 설비 POC 예외 처리
            if pos[2] < 49000:
                continue
            #endregion
...
def check_bim_info_rule_set(bim_info : list[BIMInfo]) -> list[BIMInfo]:
    ...
    for bim in bim_info:
        ...
        #region H-Beam 예외처리
        if max_data[2] > 49000 and min_data[2] < 52000:
            continue
        #endregion

        # region DUCT 플랜지 양변 OOmm 이격 설정
        if "FLANGE_Central_1" in project:
            min_data = (
                min_data[0] - 50,
                min_data[1] - 50,
                min_data[2] - 50
        ...
        #region 상부 입상 시 천장 판넬로부터 OOmm 이격
...
def check_rule_set(
    ...
    for i in range(3):
        ...
        # region DUCT 외벽으로부터 OOmm 이격거리 확보
    ...
    # region DUCT POC 수직방향 OOmm 내 장애물 간섭시 예외 처리
    min_check_distance = 400
    #endregion
...
def get_report_entities(entities : list[RoutingEntity], output_folder : str, time : float) -> None:

    duct_poc_box_data : list[dict] = []

    success_num = 0
    fail_num = 0
    fail_data : set[str] = set()
    success_data : set[str] = set()

    result_entities : list[RoutingEntity] = []
    fail_entities : list[RoutingEntity] = []

    fail_equip_box : list[dict] = []
    ...
    for entity in entities:
        ...
        utility = entity["attr"]["utility"]


        if duct_poc == (0,0,0):
            fail_num += 1
            fail_data.add(f"{equip_id} - {utility}")

            equip_poc_box = get_box(entity["end"], diameter)
            fail_equip_box.append({
                "min" : tuple_to_string(equip_poc_box[0]),
                "max" : tuple_to_string(equip_poc_box[1])
            })
            fail_entities.append(entity)
            continue
        success_num += 1 
        success_data.add(f"{equip_id} - {utility}")

        duct_poc_box = get_box(duct_poc,diameter)

        duct_poc_box_data.append({
            "min" : Importer.tuple_to_string(duct_poc_box[0]),
            "max" : Importer.tuple_to_string(duct_poc_box[1])
        })

        result_entities.append(entity)
        result_entities[-1]["path"] = [
            equip_poc,duct_poc
        ]

    with open(f"{output_folder}\\duct_poc_box.json", "w", encoding="utf-8") as make_file:
        json.dump({"obstacles" : duct_poc_box_data}, make_file, indent=4)

    with open(f"{output_folder}\\fail_equip_box.json", "w", encoding="utf-8") as make_file:
        json.dump({"obstacles" : fail_equip_box}, make_file, indent=4)


    with open(f"{output_folder}\\point_of_connectors.json", "w", encoding="utf-8") as make_file:
        json.dump(result_entities, make_file, indent=4)


    with open(f"{output_folder}\\fail_entities.json", "w", encoding="utf-8") as make_file:
        json.dump(fail_entities, make_file, indent=4)

    report = open(f"{output_folder}\\report.txt","w",encoding="utf_8")
    report.write(f"process time : {time}\n")
    report.write(f"equip poc 수 : {len(entities)}\n")
    report.write(f"duct poc 생성 성공 수 : {success_num}\n")
    report.write(f"duct poc 생성 실패 수 : {fail_num}\n")

    report.write("\nduct poc 생성 성공 장비 리스트 :\n")
    index = 0
    for string in success_data:
        index += 1
    ...
    report.write("\nduct poc 생성 실패 장비 리스트 :\n")
    index = 0
    for string in fail_data:
        index += 1
        report.write(f"{string}, ")
        if index == 5:
            index = 0
...
def get_bim_output(bim_info : list[BIMInfo], output_folder : str) -> None:
    utility_data : list[str]= ["ACID","HEAT","CABINET"]
    duct_bim_info : list[BIMInfo] = []
    ...
    duct_bim_data : list[dict] = []

    for bim in duct_bim_info:
        bim_bb_box = bim["bounding_box"]
        duct_bim_data.append({
            "min" : Importer.tuple_to_string(bim_bb_box[0]),
            "max" : Importer.tuple_to_string(bim_bb_box[1])
        })

    with open(f"{output_folder}\\duct.json", "w", encoding="utf-8") as make_file:
        json.dump({"obstacles" : duct_bim_data}, make_file, indent=4)

    bim_obstacles : list[dict] = []
    for bim in bim_info:
        bim_bb_box = bim["bounding_box"]
        bim_obstacles.append({
            "min" : Importer.tuple_to_string(bim_bb_box[0]),
            "max" : Importer.tuple_to_string(bim_bb_box[1])
        })

    with open(f"{output_folder}\\check_bim_obstacles.json", "w", encoding="utf-8") as make_file:
        json.dump({"obstacles" : bim_obstacles}, make_file, indent=4)
...
def input_vaildation_fn(point_of_connectors_folder : str, bim_info_path : str, output_folder : str) -> tuple[list[RoutingEntity],list[dict]]:
    ...
    # region 설비 POC간 간섭으로 인한 설비POC 강제 이동 리스트
    move_data = [
        [[185371.51,	 94339.68,	    49500.0	        ],[0	 ,0	 -1,],	[185371.51,	 94389.68	 ,49500.0	],[0	 ,0	, -1]],
        [[184423.51,	 94339.68,	    49500.0	        ],[0	 ,0	 -1,],	[184423.51,	 94389.68	 ,49500.0	],[0	 ,0	, -1]],
        [[184550.79,	 94179.68,	    49498.72	    ],[0	 ,0	 -1,],	[184600.79,	 94179.68	 ,49498.72	],[0	 ,0	, -1]],
        [[185498.79,	 94179.68,	    49498.72	    ],[0	 ,0	 -1,],	[185548.79,	 94179.68	 ,49498.72	],[0	 ,0	, -1]],
        [[185065.23,	 92772.68,	    49498.72	    ],[0	 ,0	 -1,],	[185015.23,	 92772.68	 ,49498.72	],[0	 ,0	, -1]],
        [[186013.23,	 92772.68,	    49498.72	    ],[0	 ,0	 -1,],	[185963.23,	 92772.68	 ,49498.72	],[0	 ,0	, -1]],
        [[186140.51,	 92612.68,	    49500.0	        ],[0	 ,0	 -1,],	[186140.51,	 92562.68	 ,49500.0	],[0	 ,0	, -1]],
        [[185192.51,	 92612.68,	    49500.0	        ],[0	 ,0	 -1,],	[185192.51,	 92562.68	 ,49500.0	],[0	 ,0	, -1]],
        [[177370.42,	 91976.1	,   49500.0	        ],[0	 ,0	 -1,],	[177370.42,	 92026.1	 ,49500.0	],[0	 ,0	, -1]],
        [[178318.42,	 91976.1	,   49500.0	        ],[0	 ,0	 -1,],	[178318.42,	 92026.1	 ,49500.0	],[0	 ,0	, -1]],
        [[177497.7	, 91816.1	 ,      49498.72	    ],[0	 ,0	 -1,],	[177547.7	, 91816.1	 ,49498.72	],[0	 ,0	, -1]],
        [[178445.7	, 91816.1	 ,      49498.72	    ],[0	 ,0	 -1,],	[178495.7	, 91816.1	 ,49498.72	],[0	 ,0	, -1]],
        [[178012.14,	 90409.1	,   49498.72	    ],[0	 ,0	 -1,],	[177962.14,	 90409.1	 ,49498.72	],[0	 ,0	, -1]],
        [[178960.14,	 90409.1	,   49498.72	    ],[0	 ,0	 -1,],	[178910.14,	 90409.1	 ,49498.72	],[0	 ,0	, -1]],
        [[178139.42,	 90249.1	,   49500.0	        ],[0	 ,0	 -1,],	[178139.42,	 90199.1	 ,49500.0	],[0	 ,0	, -1]],
        [[179087.42,	 90249.1	,   49500.0	        ],[0	 ,0	 -1,],	[179087.42,	 90199.1	 ,49500.0	],[0	 ,0	, -1]],
        [[206552.37,	 88121.14,	    50228.0	        ],[0	 ,0	 -1,],	[206552.37,	 88121.14	 ,50228.0	],[0	 ,1	, 0  ]],    
        [[206563.63,	 88120.44,	    50103.77	    ],[0	 ,0	 -1,],	[206513.63,	 88120.44	 ,50103.77	],[0	 ,1	, 0  ]],    
        [[206613.63,	 88120.44,	    50007.7	        ],[0	 ,0	 -1,],	[206613.63,	 88120.44	 ,49957.7	],[0	 ,1	, 0  ]],        
        [[206668.63,	 88120.44,	    50103.77	    ],[0	 ,0	 -1,],	[206718.63,	 88120.44	 ,50103.77	],[0	 ,1	, 0  ]],
        [[206552.35,	 69825.53,	    50228.0	        ],[0	 ,0	 -1,],	[206552.35,	 69825.53	 ,50228.0	],[0	 ,1	, 0  ]],
        [[206563.61,	 69824.83,	    50103.77	    ],[0	 ,0	 -1,],	[206513.61,	 69824.83	 ,50103.77	],[0	 ,1	, 0  ]],
        [[206613.61,	 69824.83,	    50007.7	        ],[0	 ,0	 -1,],	[206613.61,	 69824.83	 ,49957.7	],[0	 ,1	, 0  ]],
        [[206668.61,	 69824.83,	    50103.77	    ],[0	 ,0	 -1,],	[206718.61,	 69824.83	 ,50103.77	],[0	 ,1	, 0  ]],
        [[206549.43,	 65383.15,	    50227.48	    ],[0	 ,0	 -1,],	[206549.43,	 65383.15	 ,50227.48	],[0	 ,1	, 0  ]],
        [[206555.64,	 65382.82,	    50103.75	    ],[0	 ,0	 -1,],	[206505.64,	 65382.82	 ,50103.75	],[0	 ,1	, 0  ]],
        [[206665.64,	 65382.82,	    50103.75	    ],[0	 ,0	 -1,],	[206715.64,	 65382.82	 ,50103.75	],[0	 ,1	, 0  ]],
        [[206610.64,	 65382.82,	    50007.68	    ],[0	 ,0	 -1,],	[206610.64,	 65382.82	 ,49957.68	],[0	 ,1	, 0  ]],
        [[206549.43,	 61075.19,	    50227.48	    ],[0	 ,0	 -1,],	[206549.43,	 61075.19	 ,50227.48	],[0	 ,1	, 0  ]],
        [[206555.64,	 61074.86,	    50103.75	    ],[0	 ,0	 -1,],	[206505.64,	 61074.86	 ,50103.75	],[0	 ,1	, 0  ]],
        [[206665.64,	 61074.86,	    50103.75	    ],[0	 ,0	 -1,],	[206715.64,	 61074.86	 ,50103.75	],[0	 ,1	, 0  ]],
        [[206610.64,	 61074.86,	    50007.68	    ],[0	 ,0	 -1,],	[206610.64,	 61074.86	 ,49957.68	],[0	 ,1	, 0  ]]
    ]
    #endregion
    for entity in entities:
        equip_poc = entity["start"]
    ...
    report_json_data = get_report(entities, duct_data_storage)
